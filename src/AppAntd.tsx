...
import type { IData_SnippetNews } from './data/data.ts';
...
export default function AppAntd() {
  ...
  return (
    <Layout className="layout" style={{ minHeight: '100vh' }}>
      <Header>
        <div className="logo" style={{ float: 'left', color: 'white', fontWeight: 'bold', fontSize: '20px' }}>
          Kaspersky News
        </div>
      ...
      <Content style={{ padding: '0 50px', marginTop: '24px' }}>
        <Card title={data.TI} bordered={false} style={{ maxWidth: 800, margin: 'auto' }}>
          <Typography>
            <Paragraph>{data.AB}</Paragraph>
            <Paragraph>
              Keywords: {' '}
              {data.KW.map((kw) => (
                <Tag key={kw.value} color="blue">{kw.value}</Tag>
              ))}
            </Paragraph>
            <Paragraph>
              Source: <a href={data.URL} target="_blank" rel="noopener noreferrer">{data.DOM}</a>
            </Paragraph>
          </Typography>
        </Card>
      </Content>
      <Footer style={{ textAlign: 'center' }}>Kaspersky ©2025 Created by Kaspersky Task</Footer>
    </Layout>
  );
}
